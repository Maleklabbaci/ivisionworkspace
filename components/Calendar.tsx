...
const Calendar: React.FC<CalendarProps> = ({ tasks, users, currentUser, onAddTask, onUpdateStatus }) => {
  ...
  return (
    <div className="flex flex-col h-full space-y-8 page-transition pb-24 no-scrollbar">
      {/* HEADER CALENDRIER - Aligné sur le screenshot */}
      <div className="flex items-start justify-between px-2 pt-2">
        ...
        <div className="flex items-center space-x-2">
          <button onClick={prevMonth} className="p-4 bg-white border border-slate-50 rounded-3xl text-slate-400 active-scale shadow-sm transition-all hover:bg-slate-50">
            <ChevronLeft size={22} />
          </button>
          <button onClick={nextMonth} className="p-4 bg-white border border-slate-50 rounded-3xl text-slate-400 active-scale shadow-sm transition-all hover:bg-slate-50">
            <ChevronRight size={22} />
          </button>
        </div>
      </div>

      {/* GRILLE CALENDRIER - Design épuré du screenshot */}
            ...
            return (
              <div key={date.toISOString()} className="flex flex-col items-center justify-center">
                <button
                  ...
                  `}
                >
                  <span className={`text-sm font-black ${isSelected(date) ? 'text-white' : ''}`}>
                    {date.getDate()}
                  </span>
                </button>
      ...
      {/* SECTION TACHES - Aligné sur le screenshot */}
      <div className="space-y-6">
        <div className="flex items-center justify-between px-3">
          ...
          <button 
            ...
            className="flex items-center px-5 py-3 bg-primary/5 text-primary text-[10px] font-black rounded-2xl active-scale tracking-widest uppercase transition-all hover:bg-primary/10"
          >
            <Plus size={14} className="mr-2" strokeWidth={3} /> AJOUTER
          </button>
        </div>
              ...
              <div key={task.id} className="bg-white p-6 rounded-[2.5rem] border border-slate-50 shadow-sm flex items-center justify-between transition-all hover:shadow-md">
                <div className="flex items-center space-x-5 overflow-hidden">
                  ...
                  <div className="overflow-hidden">
                    ...
                    <p className="text-[10px] text-slate-300 font-black uppercase mt-1 tracking-widest flex items-center">
                      {task.type} <span className="mx-2 opacity-50">•</span> {task.status.toUpperCase()}
                    </p>
                  </div>
                </div>
                <button 
                  onClick={(e) => { e.stopPropagation(); onUpdateStatus(task.id, TaskStatus.DONE); }} 
                  className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${task.status === TaskStatus.DONE ? 'bg-success/10 text-success' : 'bg-slate-50 text-slate-200 hover:text-primary hover:bg-primary/5'}`}
                >
                  <CheckCircle size={24} />
                </button>
              </div>
      ...
      {/* FORM MODAL - Z-Index 100 pour couvrir la nav */}
      {showAddModal && (
          ...
          <div className="bg-white rounded-t-[48px] md:rounded-[48px] shadow-2xl w-full max-w-md overflow-hidden animate-in slide-in-from-bottom duration-500 p-10 pb-[calc(2.5rem+env(safe-area-inset-bottom))]">
            ...
            <div className="space-y-6">
              <div>
                ...
                <input 
                  type="text" 
                  className="w-full p-6 bg-slate-50/80 border border-slate-100 rounded-3xl font-black text-slate-900 focus:bg-white focus:border-primary/20 outline-none transition-all placeholder-slate-300" 
                  placeholder="Ex: Réunion stratégique"
                  id="task-title-cal-opt"
                />
              </div>
              <button 
                onClick={() => {
                  const input = document.getElementById('task-title-cal-opt') as HTMLInputElement;
                ...
                className="w-full py-6 bg-primary text-white font-black rounded-3xl shadow-2xl shadow-primary/30 active-scale uppercase text-xs tracking-widest transition-all hover:bg-blue-700"
              >
