...
const Projects: React.FC<any> = ({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (viewMode === 'edit' && selectedProjectId) {
      onUpdateProject({ ...formData, id: selectedProjectId } as Project, activeConfigs);
    } else {
      onAddProject({ ...formData, spentBudget: 0 }, activeConfigs);
    }
    closeModal();
  };
    ...
    <div className="space-y-10 animate-fade-in pb-20">
      <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 px-2">
        ...
        <div className="flex items-center space-x-3">
          <div className="relative group">
            ...
            <input type="text" placeholder="RECHERCHER..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full lg:w-72 pl-14 pr-8 py-5 bg-white/5 border border-white/10 rounded-full text-[11px] font-black uppercase text-white outline-none focus:border-emerald-400 transition-all placeholder-slate-700" />
          </div>
          {isAdmin && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          ...
          { label: 'Valeur Totale', val: `${globalStats.totalValue.toLocaleString()} DZD`, icon: Target, color: 'text-sky-400', bg: 'bg-sky-400/10' },
          ...
          <div key={i} className="glass-card p-8 rounded-[2.5rem] flex items-center justify-between group">
            <div><p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{s.label}</p><h4 className={`text-2xl font-black ${s.color} mt-2 tracking-tighter leading-none`}>{s.val}</h4></div>
            <div className={`w-14 h-14 rounded-2xl ${s.bg} flex items-center justify-center ${s.color} group-hover:scale-110 transition-transform`}><s.icon size={24} /></div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {projectsWithFinancials.map(project => {
          ...
          return (
            <div key={project.id} className="bg-slate-900/40 backdrop-blur-3xl p-10 rounded-[3rem] border border-white/5 relative overflow-hidden group hover:border-emerald-400/20 transition-all">
              ...
              {isAdmin && (
                <div className="pt-6 border-t border-white/5 flex justify-end space-x-3 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <button onClick={() => confirm('Supprimer ?') && onDeleteProject(project.id)} className="p-3 glass rounded-xl text-slate-600 hover:text-rose-400 active-scale transition-all"><X size={18} /></button>
                </div>
              )}
            </div>
          );
      ...
      <Modal isOpen={viewMode === 'add' || viewMode === 'edit'} onClose={closeModal} title="Projet Architecture" subtitle="Matrice Opérationnelle iV">
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="space-y-5">
            <div><label className="label-iv"><Type size={12} className="text-emerald-400"/> Désignation</label><input required className="input-iv" placeholder="Nom du projet" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} /></div>
            ...
            <div className="p-5 md:p-8 bg-emerald-400/5 rounded-[2rem] border border-emerald-400/10 space-y-6">
              ...
              <div className="space-y-4 max-h-[400px] overflow-y-auto no-scrollbar pr-1">
                {autoBatches.map((batch, index) => (
                  <div key={batch.id} className="p-4 bg-slate-950/40 rounded-3xl border border-white/5 space-y-4 animate-slide-up relative group/batch">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                    ...
                    {batch.enabled && (
                      <div className="space-y-4 animate-fade-in">
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          <div className="space-y-1"><label className="label-iv text-[8px] opacity-50">Type / Préfixe</label><input className="input-iv h-10 bg-slate-900/50" placeholder="Ex: Vidéo" value={batch.prefix} onChange={e => updateBatch(batch.id, { prefix: e.target.value })} /></div>
                        </div>
                        <div className="flex flex-wrap gap-1.5 pt-1">
                          {presets.map(p => (
                            <button key={p} type="button" onClick={() => updateBatch(batch.id, { prefix: p })} className={`px-2.5 py-1.5 rounded-lg text-[7px] font-black uppercase transition-all ${batch.prefix === p ? 'bg-emerald-400 text-slate-950 shadow-lg' : 'bg-white/5 text-slate-600 hover:bg-white/10'}`}>{p}</button>
                          ))}
                        </div>
          ...
          <button type="submit" className="w-full py-6 bg-emerald-400 text-slate-950 font-black rounded-3xl shadow-xl active-scale uppercase text-[10px] tracking-widest mt-4 hover:bg-emerald-300 transition-all flex items-center justify-center">
            ...
            {viewMode === 'add' ? 'Lancer le Projet' : 'Sauvegarder les Changements'}
          </button>
        </form>
      </Modal>
    </div>
