...
const Clients: React.FC<any> = ({ clients = [], tasks = [], projects = [], salaries = [], expenses = [], adCampaigns = [], fileLinks = [], onAddClient, onUpdateClient, onDeleteClient, currentUser }) => {
  ...
  return (
    <div className="relative">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 px-2">
          ...
          <div className="flex items-center space-x-3 w-full lg:w-auto">
             <div className="relative flex-1 lg:w-80">
               ...
               <input type="text" placeholder="RECHERCHER..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-14 pr-6 py-4 bg-white/5 border border-white/10 rounded-full text-[11px] font-black uppercase tracking-widest text-white outline-none focus:border-emerald-400/50 focus:bg-white/10 transition-all placeholder-slate-700" />
             </div>
             {canManage && (
               <button onClick={() => { setFormData({ name: '', company: '', email: '', phone: '', address: '', description: '' }); setViewMode('add'); }} className="w-14 h-14 bg-emerald-400 text-slate-950 rounded-2xl shadow-xl shadow-emerald-500/20 active-scale flex items-center justify-center transition-all hover:scale-105">
      ...
      <Modal isOpen={viewMode === 'view' && !!selectedClient} onClose={closeModals} title={selectedClient?.name} subtitle={selectedClient?.company || 'Profil Partenaire'}>
        <div className="space-y-10">
           ...
           <div className="flex flex-col sm:flex-row items-center gap-4 pt-4">
              {canManage && (
                <button onClick={() => handleOpenEdit(selectedClient!)} className="w-full py-6 bg-emerald-400 text-slate-950 font-black rounded-[2rem] shadow-xl active-scale uppercase text-[11px] tracking-widest hover:bg-emerald-300 flex items-center justify-center space-x-3">
                  <Edit2 size={18} />
      ...
      <Modal isOpen={viewMode === 'add' || viewMode === 'edit'} onClose={closeModals} title={viewMode === 'add' ? 'Nouveau Partenaire' : 'Modifier Partenaire'} subtitle="Mise à jour du Système CRM">
        <form onSubmit={(e) => { 
          e.preventDefault(); 
