...
import { TrendingUp, Target, Zap, Clock, ChevronRight, Activity, BarChart3, ListChecks, AlertTriangle, HelpCircle, AlertCircle, Calendar } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ currentUser, tasks = [], clients = [], onNavigate }) => {
  ...
  const displayFirstName = useMemo(() => {
    if (!currentUser?.name) return "Agent iV";
  ...
  const urgentAlertTasks = useMemo(() => {
    return relevantTasks
      ...
      .sort((a, b) => (a.priority === 'high' ? -1 : 1) || a.dueDate.localeCompare(b.dueDate))
      .slice(0, 3);
  }, [relevantTasks, today]);
  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-fade-in">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4 px-1">
        <div className="flex items-start space-x-4">
          <div>
            ...
            <h1 className="text-2xl md:text-5xl font-black text-white tracking-tighter uppercase leading-none text-left">Bonjour, {displayFirstName} 👋</h1>
          </div>
         ...
         <div className="space-y-6 text-left">
            ...
            <div className="space-y-4">
               <div className="p-4 bg-white/5 rounded-2xl border border-white/5">
                  ...
                  <p className="text-slate-400 text-xs">Utilisez @NomMission + Action (urgent, bloquer, terminé) dans le chat pour piloter vos flux.</p>
               </div>
               <div className="p-4 bg-rose-500/10 rounded-2xl border border-rose-500/20">
                  <h4 className="text-rose-500 font-bold text-xs uppercase mb-2">Alerte Rouge iV</h4>
                  <p className="text-slate-400 text-xs">Tout blocage critique (statut BLOQUÉ) place l'agence en alerte visuelle prioritaire.</p>
               </div>
            </div>
         </div>
      </Modal>
          ...
          <div className="bg-rose-500 border-4 border-rose-400 p-8 md:p-10 rounded-[2.5rem] md:rounded-[3.5rem] flex flex-col md:flex-row items-center justify-between gap-6 shadow-[0_0_60px_rgba(244,63,94,0.4)] ring-8 ring-rose-500/10">
            <div className="flex items-center space-x-6 text-left">
              ...
              <div>
                ...
                <p className="text-[11px] md:text-[13px] font-black text-white uppercase tracking-[0.2em] mt-3 opacity-90">{blockedTasks.length} MISSION(S) INTERROMPUE(S)</p>
              </div>
            </div>
            <button onClick={() => onNavigate('tasks')} className="px-10 py-5 bg-white text-rose-500 rounded-2xl font-black uppercase text-[11px] tracking-widest shadow-2xl active-scale">Débloquer le Flux</button>
          </div>
        </div>
      )}

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 px-1">
        {stats.filter(s => s.allowed).map((s, i) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 md:gap-6 px-1 text-left">
        <div className="lg:col-span-2 space-y-5 md:space-y-6">
          <div onClick={() => onNavigate('tasks')} className="relative group cursor-pointer overflow-hidden rounded-[2rem] md:rounded-[3rem] border border-white/10 p-1 bg-gradient-to-br from-white/5 to-transparent active-scale transition-all shadow-lg">
            <div className="bg-slate-950/40 backdrop-blur-3xl p-6 md:p-12 rounded-[1.9rem] md:rounded-[2.9rem] flex items-center justify-between">
              <div className="flex items-center space-x-5 md:space-x-8">
                ...
                <div>
                  <h3 className="text-lg md:text-2xl font-black text-white uppercase tracking-tight leading-none">Missions Actives</h3>
                  <p className="text-slate-500 text-[9px] mt-1.5 font-bold uppercase tracking-tight">Suivi de votre production opérationnelle</p>
                </div>
              </div>
          ...
          <div className="space-y-3 md:space-y-4">
            <h3 className="text-[10px] font-black uppercase text-slate-500 tracking-widest px-3">Flux Prioritaire</h3>
              ...
              {relevantTasks.filter(t => t.status !== TaskStatus.DONE).slice(0, 4).map(task => (
                <div key={task.id} onClick={() => onNavigate('tasks')} className={`crystal-module p-5 rounded-[1.5rem] md:rounded-[2rem] border-l-[4px] active-scale flex items-center justify-between group border-white/5 ${task.status === TaskStatus.BLOCKED ? 'border-l-rose-500 bg-rose-500/10' : 'border-l-sky-500'}`}>
                  <div className="truncate pr-4 flex-1">
                    <h4 className={`font-bold text-[12px] uppercase truncate transition-colors tracking-tight ${task.status === TaskStatus.BLOCKED ? 'text-rose-400' : 'text-white'}`}>{task.title}</h4>
                    <p className="text-[9px] text-slate-600 font-bold mt-1.5 uppercase tracking-tighter">{task.dueDate}</p>
                  </div>
                  <div className={`px-2 py-1 rounded-lg text-[8px] font-black uppercase flex-shrink-0 border ${task.status === TaskStatus.BLOCKED ? 'bg-rose-500 text-white' : 'bg-white/5 text-slate-400'}`}>
                    {task.status === TaskStatus.BLOCKED ? 'STOP' : 'MED'}
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-6">
          <div className="crystal-module p-10 md:p-12 rounded-[2.5rem] md:rounded-[3.5rem] flex flex-col items-center justify-center text-center space-y-8 relative overflow-hidden shadow-xl border-white/5">
            <div className="w-40 h-40 md:w-48 md:h-48 rounded-full border-[10px] border-white/5 border-t-sky-400 flex items-center justify-center shadow-[0_0_40px_rgba(14,165,233,0.1)]">
                 <span className="text-4xl md:text-5xl font-black text-white tracking-tighter">{productivityScore}%</span>
            </div>
            <div>
              <h4 className="text-xl md:text-2xl font-black text-white uppercase tracking-tight leading-none">Rendement iV</h4>
              <p className="text-slate-600 text-[10px] mt-2 font-black uppercase">Efficacité calculée</p>
            </div>
            <button onClick={() => onNavigate('tasks')} className="w-full py-4 bg-white text-slate-950 rounded-2xl font-black text-[10px] uppercase active-scale shadow-2xl hover:bg-sky-400 hover:text-white transition-all tracking-widest">Vérifier Missions</button>
          </div>

          {urgentAlertTasks.length > 0 && (
            <div className="space-y-4">
              <h3 className="text-[10px] font-black uppercase text-rose-500 tracking-widest px-2 flex items-center"><AlertTriangle size={14} className="mr-2" /> Alertes</h3>
              <div className="space-y-3">
                {urgentAlertTasks.map(task => (
                  <div key={task.id} onClick={() => onNavigate('tasks')} className="p-4 rounded-[1.5rem] bg-rose-500/5 border border-rose-500/20 flex items-center space-x-4 group cursor-pointer active-scale transition-all">
                    <div className="w-10 h-10 rounded-xl bg-rose-500/20 flex items-center justify-center text-rose-500"><Calendar size={18} /></div>
                    <div className="min-w-0 flex-1">
                      <p className="text-[12px] font-black text-white uppercase truncate">{task.title}</p>
                      <p className="text-[9px] font-bold text-slate-600 uppercase mt-1">{task.status === TaskStatus.BLOCKED ? 'Bloqué' : `Retard (${task.dueDate})`}</p>
                    </div>
                  </div>
                ))}
              </div>
