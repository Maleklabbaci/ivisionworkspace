    ...
    <Layout currentUser={currentUser} onLogout={handleLogout}>
      <Suspense fallback={<div className="h-full w-full flex items-center justify-center"><Loader2 className="animate-spin text-primary" size={40} /></div>}>
        <Routes>
          ...
          <Route path="/tasks" element={<Tasks tasks={tasks} users={users} clients={clients} projects={projects} currentUser={currentUser} onUpdateStatus={(id:string,st:any)=> { supabase.from('tasks').update({status:st}).eq('id',id).then(()=>setTasks(tasks.map(t=>t.id===id?{...t,status:st}:t))); }} onBatchUpdateStatus={(ids:string[], st:any) => {
            supabase.from('tasks').update({status: st}).in('id', ids).then(() => {
              setTasks(tasks.map(t => ids.includes(t.id) ? {...t, status: st} : t));
              addNotification("Système", `${ids.length} missions mises à jour.`, "success");
            });
          }} onBatchUpdateTasks={(ids: string[], updates: Partial<Task>) => {
            const dbUpdates: any = {};
            ...
            supabase.from('tasks').update(dbUpdates).in('id', ids).then(() => {
              setTasks(tasks.map(t => ids.includes(t.id) ? { ...t, ...updates } : t));
